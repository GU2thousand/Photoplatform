...
- Controlled recovery: 6 integration checks passed for expired lease recovery, delete before worker claim, historical-version retry isolation, unknown-image retry returning 404, late PUT staging resweep, and hourly deleted-prefix reconciliation. These deliberately manipulate disposable database timestamps/locks; they are not a claim of broad production chaos testing.
- Real RabbitMQ outage: stopped the disposable broker container, completed an upload while publication was unavailable, verified its QUEUED job and unconfirmed outbox row, restarted the broker, and observed successful processing with exactly five variants.
...
- Real Chrome: registration, private object-store PUT, automatic PROCESSING→READY updates, decoded signed thumbnails/originals, private keyword search and logout cleanup passed with zero console errors. Anonymous semantic/hybrid public search also displayed real decoded results; the first hybrid browser check during the initial load test took 59 seconds, which exposed the performance issue described below.
...
|---|---:|---:|---:|---:|
| Keyword | 0.120 | 0.240 | 0.240 | 28.49 |
| Semantic | 0.434 | 0.694 | 0.763 | 53.77 |
| Hybrid RRF | 0.446 | 0.766 | 0.817 | 65.33 |
...
## Concurrent API and direct-upload load

k6 ran 20/50/100 virtual users against real HTTP APIs and object storage. Gallery/team/semantic scenarios ran for 15 seconds each with 50 ms think time; upload scenarios ran one complete create→PUT→complete burst per VU. Upload counts below include registration and all three HTTP operations. The upload fixture is a 640×480 JPEG; this is concurrency verification, not a large-file bandwidth benchmark. The disposable API allowed 1,000 active uploads per account (production default: 20).

| Scenario | VUs | HTTP requests | HTTP p50 (ms) | HTTP p95 (ms) | HTTP errors |
|---|---:|---:|---:|---:|---:|
| gallery | 20 | 1669 | 66.3 | 481.9 | 0.0% |
| gallery | 50 | 1273 | 520.3 | 1071.5 | 0.0% |
| gallery | 100 | 2448 | 414.3 | 1416.0 | 0.0% |
| team | 20 | 2198 | 37.3 | 221.8 | 0.0% |
| team | 50 | 2047 | 227.8 | 999.0 | 0.0% |
| team | 100 | 3714 | 279.4 | 994.1 | 0.0% |
| semantic | 20 | 836 | 220.9 | 870.6 | 0.0% |
| semantic | 50 | 1701 | 364.9 | 751.7 | 0.0% |
| semantic | 100 | 2736 | 430.7 | 1101.2 | 0.0% |
| upload | 20 | 61 | 222.2 | 545.8 | 0.0% |
| upload | 50 | 151 | 600.6 | 1554.3 | 0.0% |
| upload | 100 | 301 | 1623.1 | 4200.1 | 0.0% |

Gallery and team scopes contained 50 CIFAR images each; semantic search used the public scope, 24 hits and a repeated warm query. API JVM heap was 512 MiB and Hikari's default pool was 10 connections. OpenTelemetry tracing and the monitoring stack were active. All services shared the same development machine with unrelated work; these short single runs have no confidence intervals or production SLO claim.

**The first semantic load failed.** Per-hit response hydration opened a transaction for every result. At 50 VUs, the completed requests had 100% HTTP failure and many iterations exceeded the test grace period; 100 VUs had 9% HTTP failure and approximately 44-second p95. The fix loads image/author/team entities together and hydrates all hits in one transaction, retaining the same access checks. Model HTTP requests remain outside that transaction. The 20/50/100-VU semantic reruns above all had zero errors; 100-VU p95 was approximately 1.10 seconds. The 20-VU rerun overlapped a brief local compilation; treat timing differences between rows cautiously.

Both [initial raw summaries](benchmarks/initial-load/) and [optimized semantic summaries](benchmarks/optimized-load/) are retained. No claim is made that this proves 100 simultaneous large-file uploads, geographically distributed CDN performance, or cold unique-query encoder throughput. Batch cleanup also now prioritizes the earliest due cleanup time, so a backlog of first cleanups does not starve late-write resweeps.
...
[GitHub Actions run 35564186868](https://github.com/GU2thousand/Photoplatform/actions/runs/35564186868) passed all four jobs: Java 17 backend, frontend build/tests/audit, worker + metric tests, and real Docker pipeline/recovery/legacy migration. Later housekeeping and broker-outage coverage are verified again on the final branch revision.
