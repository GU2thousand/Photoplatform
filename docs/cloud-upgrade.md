...
S3 versioning protects against accidental overwrites but changes deletion semantics: ordinary `DeleteObject` creates delete markers and retains noncurrent versions. The worker enumerates and deletes all versions and delete markers under each tombstoned `media/<id>/` prefix before marking it deleted. Its IAM principal requires `s3:ListBucketVersions` on the bucket and `s3:DeleteObjectVersion` on the media prefix (in addition to normal read/write permissions). Failures remain retryable with capped backoff; hourly reconciliation also recovers old dead-lettered deletions. S3-compatible providers must support these version APIs even for unversioned buckets. The sample expires staging versions after seven days; that staging retention window is separate from media-prefix erasure. Delete markers, replication, object lock, and independent backups all affect actual erasure; do not label an application `DELETED` state as proof of physical removal from every copy.
